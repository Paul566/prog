...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.sin(self.nodes[0, :] / 10)
    ...
    def move(self, tau, t, T):
        # По сути метод просто двигает все точки c их текущими скоростями
        self.nodes += self.velocity * tau
        for i in range(len(self.nodes[0])):
            self.smth[i] = np.sin((self.nodes[2][i] + t) / 10)
            if self.nodes[2][i] > 44:
                self.velocity[0][i] = self.nodes[0][i] * np.sin(2 * np.pi * t / T) * (80 - self.nodes[2][i]) / 1000
                self.velocity[1][i] = self.nodes[1][i] * np.sin(2 * np.pi * t / T) * (80 - self.nodes[2][i]) / 1000
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("breathingknight-step-" + str(snap_number) + ".vtu")
        writer.Write()
